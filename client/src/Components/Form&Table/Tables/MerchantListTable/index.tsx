...
import { MerchantListFilterHeader } from "./MerchantListFilterHeader";
import { CollapseFilterData } from "./CollapseFilterData";
import { MerchantColumn, MerchantList } from "./MerchantListTableData";


const MerchantListContainer = () => {
  const [filterText, setFilterText] = useState("");
  const [filteredItems, setfilteredItems] = useState(MerchantList)

  // MerchantList.filter((item) => item.name && item.name.toLowerCase().includes(filterText.toLowerCase()));

  useEffect(() => {
    filterText === ""
      ?
      setfilteredItems(MerchantList)
      :
      setfilteredItems(MerchantList.filter((item) => item.name && item.name.toLowerCase().includes(filterText.toLowerCase())))
  }, [filterText])
            ...
            <CardBody>
              <div className="d-block d-md-flex justify-content-between align-items-start gap-3">
                <div id="row_create_filter" className="dataTables_filter flex-grow-1 mb-3 mb-md-0">
                  <Input placeholder="Search Merchant . . ." onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFilterText(e.target.value)} type="search" value={filterText} className="w-100%" style={{ maxWidth: '800px' }}
                  />
                </div>
                <div className="list-product-header flex-shrink-0">
                  <MerchantListFilterHeader />
                </div>
              </div>
              <CollapseFilterData />
              <div className="list-product">
                <div className="table-responsive">
                  <DataTable data={filteredItems} persistTableHead columns={MerchantColumn} highlightOnHover striped pagination className="display dataTable theme-scrollbar tbl_custome" customStyles={{
                    rows: {
                      style: {
